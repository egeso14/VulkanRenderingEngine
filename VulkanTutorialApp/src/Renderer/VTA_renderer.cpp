...
namespace VTA
{
	...
	std::unique_ptr<VTASwapChain> VTARenderer::swapChain = nullptr;
	std::vector<VkCommandBuffer> VTARenderer::gameCommandBuffers;
	...
	void VTARenderer::beginMainFrame()
	{
	...
	VkCommandBuffer VTARenderer::beginGameCommandBuffer()
	{
		VkCommandBuffer commandBuffer = gameCommandBuffers[swapChain->getFrameIndex()];
	...
	void VTARenderer::endGameCommandBuffer()
	{
		if (vkEndCommandBuffer(getCurrentGameCommandBuffer()) != VK_SUCCESS)
		{
	...
	void VTARenderer::endMainFrame()
	{
		assert(isFrameStarted && "Can't call endFrame while frame is not in progress");
		std::vector<VkCommandBuffer> commandBuffer{ getCurrentGameCommandBuffer(), getCurrentEditorUICommandBuffer()};
	...
	void VTARenderer::createGameCommandBuffers()
	{
			gameCommandBuffers.resize(VTASwapChain::MAX_FRAMES_IN_FLIGHT); // w or three depending on whether our device supoprts triple buffering
			...
			allocInfo.commandBufferCount = static_cast<uint32_t>(gameCommandBuffers.size());

			if (vkAllocateCommandBuffers(device.device(), &allocInfo, gameCommandBuffers.data()) != VK_SUCCESS)
			{
				throw std::runtime_error("failed to allocate command buffers!");
			}
	}

	void VTARenderer::createGUICommandBuffers()
	{
		guiCommandBuffers.resize(VTASwapChain::MAX_FRAMES_IN_FLIGHT); // w or three depending on whether our device supoprts triple buffering
		// this is because we are using a seperate command buffer for each frame buffer
		VkCommandBufferAllocateInfo allocInfo{};
		allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
		allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY; // primary command buffers can be submitted to a queue, secondary command buffers can be called from primary command buffers but cannot be submitted to a queue
		allocInfo.commandPool = device.getCommandPool(); // command pools are pooling. I think this is a memory optimization
		allocInfo.commandBufferCount = static_cast<uint32_t>(guiCommandBuffers.size());

		if (vkAllocateCommandBuffers(device.device(), &allocInfo, guiCommandBuffers.data()) != VK_SUCCESS)
		{
			throw std::runtime_error("failed to allocate command buffers!");
		}
	}
	...
	void VTARenderer::beginSwapChainRenderPass1(VkCommandBuffer commandBuffer)
	{
		...
		assert(commandBuffer == getCurrentGameCommandBuffer() && "Can't begin render pass on command buffer from a different frame");


		VkRenderPassBeginInfo renderPassInfo{};
		renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
		renderPassInfo.renderPass = swapChain->getRenderPass1();
		renderPassInfo.framebuffer = swapChain->getPass1FrameBuffer(currentImageIndex); // which frame buffer is this render pass writing to?
	...
	void VTARenderer::endSwapChainRenderPass(VkCommandBuffer commandBuffer)
	{
		...
		assert(commandBuffer == getCurrentGameCommandBuffer() && "Can't end render pass on command buffer from a different frame");
	...
	{
		vkFreeCommandBuffers(device.device(), device.getCommandPool(), static_cast<float>(gameCommandBuffers.size()), gameCommandBuffers.data());
		gameCommandBuffers.clear(); // clear the command buffers vector
	}

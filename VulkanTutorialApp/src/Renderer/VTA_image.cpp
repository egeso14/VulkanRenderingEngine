...
namespace VTA_Image
{
	...
	void createImage(
		uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkSampleCountFlagBits numSample,
	...
	Texture::Texture(VTA::VTADevice& device, const Trex::Atlas& fontAtlas)
		: device(device)
	{
		createFontAtlasTextureImage(fontAtlas);
		writeToDevice();
		createFontAtlasImageView();
	...
	void Texture::createFontAtlasTextureImage(const Trex::Atlas& atlas)
	{
		pixels = (stbi_uc*)atlas.GetBitmap().Data().data();
	...
	void Texture::writeToDevice()
	{
		...
		if (mipLevels > 1)
		{
			...
			transitionImageLayout(device, textureImage, imageFormat, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mipLevels);
		}
	...
	void Texture::createFontAtlasSampler()
	{
		// specify all filters and transformations that the sampler should apply
		// magFilter specifies how to interpolate texels that are affected by oversampling
		// minFilter specifies how to interpolate texels that are affected by undersampling 

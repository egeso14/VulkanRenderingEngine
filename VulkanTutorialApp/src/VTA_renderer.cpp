...
namespace VTA
{
	...
	void VTARenderer::beginSwapChainRenderPass2(VkCommandBuffer commandBuffer)
	{
		assert(isFrameStarted && "Cannot call beginSwapChainRenderPass when frame is not in progress.");
		assert(commandBuffer == getCurrentCommandBuffer() && "Can't begin render pass on command buffer from a different frame");
		...
		VkRenderPassBeginInfo renderPassInfo{};
		renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
		renderPassInfo.renderPass = swapChain->getRenderPass2();
		renderPassInfo.framebuffer = swapChain->getPass2FrameBuffer(currentImageIndex); // which frame buffer is this render pass writing to?
		renderPassInfo.renderArea.offset = { 0, 0 }; // where to start rendering
		renderPassInfo.renderArea.extent = swapChain->getSwapChainExtent(); // swap chain extent can sometime be larger than the window extent
		std::array<VkClearValue, 2> clearValues{};
		clearValues[0].color = { 0.01f, 0.01f, 0.01f, 1.0f };
		...
		renderPassInfo.clearValueCount = static_cast<uint32_t>(clearValues.size());
		renderPassInfo.pClearValues = clearValues.data(); // only used for attachments whose loap op is clear
